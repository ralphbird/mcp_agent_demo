...
class JSONFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        if hasattr(record, "load_test_id"):
            log_entry["load_test_id"] = getattr(record, "load_test_id")  # type: ignore  # noqa: B009
        if hasattr(record, "requests_per_second"):
            log_entry["requests_per_second"] = getattr(record, "requests_per_second")  # type: ignore  # noqa: B009
        if hasattr(record, "duration_seconds"):
            log_entry["duration_seconds"] = getattr(record, "duration_seconds")  # type: ignore  # noqa: B009
        if hasattr(record, "worker_count"):
            log_entry["worker_count"] = getattr(record, "worker_count")  # type: ignore  # noqa: B009
