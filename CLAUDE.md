...
advanced monitoring, observability, and containerization practices. The project includes:

- **Currency API**: FastAPI service with conversion endpoints, health checks, and database
...
```bash
make              # Start all services with full monitoring stack (default)
make up           # Same as above - start all services
...
poetry run pytest tests/ -v --cov=currency_app --cov=load_tester --cov-report=term-missing  # Local
