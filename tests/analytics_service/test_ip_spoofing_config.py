...
class TestIPSpoofingConfigurationValidation:
    ...
    def test_geographic_regions_only_whitespace(self):
        ...
        assert "Invalid region" in error["msg"] or "At least one geographic region must be specified" in error["msg"]
...
class TestIPSpoofingEnvironmentVariables:
    ...
    def test_boolean_environment_variables(self):
        ...
        for env_value, expected in boolean_test_cases:
            ...
            try:
                settings = LoadTesterSettings()
                assert settings.ip_spoofing_enabled is expected, f"Failed for env_value: {env_value}"
            finally:
                # Restore
                if original is None:
